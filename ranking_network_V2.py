...
# ---------------------------------------------------------------------------
# Demo 1 — 3-cube toy:  F_sw -> V -> O_Ind
#
# Hand check:
#   Worlds with O_Ind=off:
#     (ok,12V,off)=INF  (ok,0V,off)=INF
#     (detached,12V,off)=INF  (detached,0V,off)=1
#   base = 1
#   F_sw=ok       | O_Ind=off:  INF - 1 = INF
#   F_sw=detached | O_Ind=off:  1   - 1 = 0
# ---------------------------------------------------------------------------
...
def three_cube_toy():
    return RankingNetwork(
        ...
        kappa_tables={
            "F_sw":  {(): {"ok": 0, "detached": 1}},
            "V":     {
                ("ok",):       {"12V": 0,   "0V": INF},
                ("detached",): {"12V": INF, "0V": 0},
            },
            "O_Ind": {
                ("12V",): {"on": 0,   "off": INF},
                ("0V",):  {"on": INF, "off": 0},
...
# ---------------------------------------------------------------------------
# Demo 2 — 4-node variant: a cable becomes a second parent of V.
# Same engine, no code changes. The point is to show reusability —
# scaling later to the 50-node DAG follows the same pattern.
#
# Switch detached has rank 1 (more believed to fail), cable broken has rank 2.
# Given O_Ind=off, both could explain it; the engine should prefer the switch.
# ---------------------------------------------------------------------------
...
def four_node_toy():
    return RankingNetwork(
        ...
        kappa_tables={
            "F_sw":    {(): {"ok": 0, "detached": 1}},
            "F_cable": {(): {"ok": 0, "broken": 2}},
            # V = 12V iff both switch and cable are ok; else 0V.
            "V": {
                ("ok",       "ok"):     {"12V": 0,   "0V": INF},
                ("ok",       "broken"): {"12V": INF, "0V": 0},
                ("detached", "ok"):     {"12V": INF, "0V": 0},
                ("detached", "broken"): {"12V": INF, "0V": 0},
            },
            "O_Ind": {
                ("12V",): {"on": 0,   "off": INF},
                ("0V",):  {"on": INF, "off": 0},
...
if __name__ == "__main__":
    print("=" * 60)
    print("Demo 1 — 3-cube toy")
    print("=" * 60)
    net3 = three_cube_toy()
    net3.validate()
    r3 = posterior(net3, query="F_sw", evidence={"O_Ind": "off"})
    print(f"posterior(F_sw | O_Ind=off) = {r3}")
    assert r3 == {"ok": INF, "detached": 0}, f"FAIL: {r3}"
    print("PASS: matches the hand calculation.")

    print()
    print("=" * 60)
    print("Demo 2 — 4-node toy with a cable parent of V")
    print("=" * 60)
    print("Same posterior() function, no code change. Different network.")
    net4 = four_node_toy()
    net4.validate()
    r4_sw    = posterior(net4, query="F_sw",    evidence={"O_Ind": "off"})
    r4_cable = posterior(net4, query="F_cable", evidence={"O_Ind": "off"})
    print(f"posterior(F_sw    | O_Ind=off) = {r4_sw}")
    print(f"posterior(F_cable | O_Ind=off) = {r4_cable}")
    assert r4_sw    == {"ok": 1, "detached": 0}, f"FAIL: {r4_sw}"
    assert r4_cable == {"ok": 0, "broken": 1},   f"FAIL: {r4_cable}"
    print("PASS")
